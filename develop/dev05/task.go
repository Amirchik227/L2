...
func init() {
	flag.IntVar(&fAfter, "A", 0, "печатать +N строк после совпадения")
	flag.IntVar(&fBefore, "B", 0, "печатать +N строк до совпадения")
	flag.IntVar(&fContext, "C", 0, "печатать ±N строк вокруг совпадения")
	flag.BoolVar(&fCount, "c", false, "количество строк")
	flag.BoolVar(&fIgnoreCase, "i", false, "игнорировать регистр")
	flag.BoolVar(&fInvert, "v", false, "вместо совпадения, исключать")
	flag.BoolVar(&fFixed, "F", false, "точное совпадение со строкой, не паттерн")
	flag.BoolVar(&fLineNum, "n", false, " печатать номер строки")
...
func myGrep(pattern string) {
	...
	if fCount {
		fmt.Println("Количество совпадений:", matchCount)
	}
}
