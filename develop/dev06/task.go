...
func init() {
	flag.StringVar(&fFields, "f", "", "выбрать поля(колонки)")
	flag.StringVar(&fDelimeter, "d", "\t", "использовать другой разделитель")
	flag.BoolVar(&fSeparated, "s", false, "только строки с разделителем")
}
